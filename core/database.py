...
def get_rate_card() -> list:
    """Return rate cards as list of dicts for validator."""
    conn = _get_conn()
...
def insert_rate_cards(rows: list) -> None:
    """Insert rate card rows (each a dict with vendor_name, item_description, unit_rate, etc.)."""
    conn = _get_conn()
    for r in rows:
        conn.execute(
            """INSERT INTO rate_cards (vendor_name, item_description, unit_rate, unit, valid_from, valid_until)
               VALUES (?, ?, ?, ?, ?, ?)""",
            (
                r.get("vendor_name"),
                r.get("item_description"),
                float(r.get("unit_rate", 0)),
                str(r.get("unit", "")),
                str(r.get("valid_from", "")),
                str(r.get("valid_until", "")),
            ),
        )
    conn.commit()
    conn.close()
...
def get_all_invoices() -> list:
    """Return all invoices as list of dicts (for duplicate check and dashboard)."""
    conn = _get_conn()
    conn.row_factory = sqlite3.Row
    rows = conn.execute("""
        SELECT id, filename, vendor_name, vendor_gstin, invoice_number, invoice_date,
               grand_total, extraction_method, extraction_confidence, processed_at
...
def get_all_flags() -> list:
    """Return all flags with invoice_id for report and dashboard."""
    conn = _get_conn()
    conn.row_factory = sqlite3.Row
    rows = conn.execute("""
        SELECT id, invoice_id, flag_type, description, billed_amount, correct_amount,
               overcharge, severity, status, created_at
        FROM flags ORDER BY id ASC
    """).fetchall()
    conn.close()
    return [dict(r) for r in rows]
...
def get_flags_for_invoice(invoice_id: int) -> list:
    ...
    conn.row_factory = sqlite3.Row
    rows = conn.execute(
        "SELECT * FROM flags WHERE invoice_id = ? ORDER BY created_at",
        (invoice_id,)
    ).fetchall()
    conn.close()
    return [dict(r) for r in rows]
...
def get_invoice_full(invoice_id: int) -> dict | None:
    """Return a single invoice including raw_json (for Audit Report page)."""
    conn = _get_conn()
    conn.row_factory = sqlite3.Row
    row = conn.execute(
...
def save_invoice(parsed: dict, flags: list, filename: str, extraction_method: str) -> int:
    """Save parsed invoice and its flags; return invoice id."""
    conn = _get_conn()
    cur = conn.execute("""
        INSERT INTO invoices (filename, vendor_name, vendor_gstin, invoice_number, invoice_date,
            grand_total, extraction_method, extraction_confidence, raw_json)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        filename,
    ...
    for f in flags:
        conn.execute("""
            INSERT INTO flags (invoice_id, flag_type, description, billed_amount, correct_amount, overcharge, severity)
            VALUES (?, ?, ?, ?, ?, ?, ?)
...
def load_demo_data():
    ...
    for inv in invoices_data:
        conn.execute("""
            INSERT OR IGNORE INTO invoices (filename, vendor_name, vendor_gstin, invoice_number, invoice_date,
                grand_total, extraction_method, extraction_confidence, raw_json)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, '{}')
        """, inv)
    ...
    for t in flags_demo:
        conn.execute("""
            INSERT OR IGNORE INTO flags (invoice_id, flag_type, description, billed_amount, correct_amount, overcharge, severity)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """, t)
